...
{% else %}
> ({{ example_id or ctx.id }}) {{ ctx.related('languageReference').name }}{{ util.references(ctx.references, with_internal_ref_link=with_internal_ref_link) }}{% if ctx.cldf.comment %} ({{ ctx.cldf.comment }}{% endif %}
<pre>
{% if ctx.data['Original_Script'] %}
{{ ctx.data['Original_Script'] }}
{% endif %}
{% if (not ctx.cldf.analyzedWord and ctx.cldf.primaryText != None) or with_primaryText %}
