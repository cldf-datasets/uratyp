...
The Majority of the 353 features are formulated as binary questions designed to be answered with “yes, this function/feature is present in the language” or “no, this function/feature is not present in the language”; some features in the GB dataset also allow for three options. The tables contain the following values: 0, 1 (in the case of multistate: 1, 2, 3) and ? (answer not known).

To ensure consistency during coding, each feature was provided with a general description and a comment explaining what should be considered when coding that particular feature. For instance, with respect to foreign influence, comments specify that very recent borrowings should not be taken into account. This and other additional information can be found in the `doc` folder and it is also included in the interactive database available at https://uralic.clld.org/. 
